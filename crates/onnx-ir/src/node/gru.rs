...
//! - **Opset 3**: Added `linear_before_reset` attribute
...
pub struct GruConfig {
    ...
    pub clip: Option<f32>,
    /// Whether to apply reset gate after matrix multiplication (linear_before_reset)
    /// - true: Matches PyTorch GRU behavior (reset_after in burn)
    /// - false: Matches original GRU paper
    ...
    /// Activation function for hidden state (default: Tanh)
...
impl NodeProcessor for GruProcessor {
    ...
    fn infer_types(
        ...
        // GRU expects 3D input: [seq_length, batch_size, input_size] or [batch_size, seq_length, input_size]
    ...
    fn extract_config(&self, node: &RawNode, _opset: usize) -> Result<Self::Config, ProcessError> {
        ...
        // Extract linear_before_reset (default: false in ONNX, but true matches PyTorch)
        // When linear_before_reset=1: apply reset gate after matrix multiplication (PyTorch style)
        // When linear_before_reset=0: apply reset gate before matrix multiplication (original paper)
